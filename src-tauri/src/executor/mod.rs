...
use fumi_executor_core::{
    default_executor_port, detect_executor_kind, is_supported_port, normalize_executor_port,
};
pub(crate) use fumi_executor_core::{executor_port_pool_for_kind, ExecutorPortPool};
pub use fumi_executor_core::{ExecutorKind, ExecutorPortSummary, ExecutorStatusPayload};
...
impl ExecutorRuntimeState {
    ...
    pub fn status<R: Runtime>(&self, app: &AppHandle<R>) -> Result<ExecutorStatusPayload> {
        ...
        let available_ports = accounts::list_executor_port_summaries(app, &executor_port_pool)?;
...
mod tests {
    ...
    use super::*;
    use fumi_executor_core::detect_executor_kind_at;
    use std::{
        fs,
    ...
    fn build_frame_matches_macsploit_layout() {
        let frame = build_frame(ExecutorIpcType::Execute, b"print('hi')").expect("frame should build");
    ...
    fn detect_executor_kind_detects_opiumware_native_dylib() {
        ...
        fs::remove_file(opiumware_native_path).expect("temp native Opiumware dylib should be removed");
    }
    ...
    fn build_opiumware_payload_prepends_the_required_script_prefix() {
        let compressed = build_opiumware_payload("print('hello')").expect("payload should compress");
