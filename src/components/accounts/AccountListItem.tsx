...
type AccountListItemProps = {
    ...
    deletingAccountId: AccountsListProps["deletingAccountId"];
    isStreamerModeEnabled: AccountsListProps["isStreamerModeEnabled"];
    launchingAccountId: AccountsListProps["launchingAccountId"];
    revealedAccountId: AccountsListProps["revealedAccountId"];
    onDeleteAccount: AccountsListProps["onDeleteAccount"];
    onHideAccount: AccountsListProps["onHideAccount"];
    onLaunchAccount: AccountsListProps["onLaunchAccount"];
    onRevealAccount: AccountsListProps["onRevealAccount"];
    onRowBlur: AccountsListProps["onRowBlur"];
};
...
export function AccountListItem({
    ...
    const accountIdentityLabel = getAccountRowIdentityLabel(account, {
        isMasked,
    });
    const accountAvatarAltText = getAccountAvatarAltText(account, {
        isMasked,
    });
    const identityBlurClassName = isMasked ? "blur-[0.20rem]" : "blur-0";

    return (
        <div className="flex flex-col justify-between gap-4 rounded-[1.35rem] border border-fumi-200 bg-fumi-50 p-4 shadow-[var(--shadow-app-card)] transition-colors hover:border-fumi-300 sm:flex-row sm:items-center">
            <div className="flex items-center gap-4">
                ...
                <div
                    ...
                    onPointerEnter={() => onRevealAccount(account.id)}
                    onPointerLeave={(event) =>
                        onHideAccount(account.id, event.currentTarget)
                    }
